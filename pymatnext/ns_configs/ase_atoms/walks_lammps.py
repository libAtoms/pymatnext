...
def walk_combined(ns_atoms, Emax, rng, walk_len, traj_info=None):
    ...
    if traj_info is not None:
        for i_avg in traj_info.get('avg_times', []):
            ...
            traj_cmd = (f"dump NS_traj_{i_avg} all custom {i_avg} {traj_info['label']}.avg_{i_avg}.lammps-dump.gz id type "
        ...
        traj_cmd = f"dump NS_traj all custom {traj_info['interval']} {traj_info['label']}.lammps-dump.gz id type x y z c_peatom"
        ns_atoms.calc.command(traj_cmd)
