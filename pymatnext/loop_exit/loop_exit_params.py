...
class NSLoopExitParams(PymatnextParams):
    ...
    module: Annotated[str | None, Field(default=None, description="module that defines an `ExitLoop` class that checks for an exit condition, with `\_\_call\_\_` method that takes current iteration and max value and returns a bool")]
